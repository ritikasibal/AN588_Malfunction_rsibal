---
title: "rsibal_FinalHomeworkCode_04.Rmd"
author: "Ritika"
date: "`r Sys.Date()`"
output:
...
if (!(n1 * p0 > 5 & n1 * (1 - p0) > 5)) {
  ...
  if(alternative == "less"){
    ...
    #https://www.r-tutor.com/elementary-statistics/hypothesis-testing/two-tailed-test-population-proportion
    if (z <= p0){
...
``` {r Question 2.3}
require(gridExtra)
pred_1 <- predict(rline_1, newdata=data.frame(kandc_data), interval = "prediction", level = 0.90) 

p1 <- ggplot(data = kandc_data, aes(x = Brain_Size_Species_Mean, y = MaxLongevity_m)) + geom_point() + geom_smooth(method = "lm", formula = y ~ x) + geom_abline(aes(slope=ci[2], intercept=ci[1], color='90% Lower Band')) + geom_abline(aes(slope=ci[4], intercept=ci[3], color='90% Upper Band')) + geom_line(aes(x = Brain_Size_Species_Mean, y = pred_1[,2]), color = '90% Upper Band Prediction') +
  geom_line(aes(x = Brain_Size_Species_Mean, y = pred_1[,3], color = '90% Lower Band Prediction'))

pred_2 <- predict(rline_2, newdata=data.frame(kandc_data), interval = "prediction", level = 0.90) 

p2 <- ggplot(data = kandc_data, aes(x = log(Brain_Size_Species_Mean), y = log(MaxLongevity_m))) + geom_point() + geom_smooth(method = "lm", formula = y ~ x) + geom_abline(aes(slope=ci_log[2], intercept=ci_log[1], color='90% Lower Band')) + geom_abline(aes(slope=ci_log[4], intercept=ci_log[3], color='90% Upper Band')) + geom_line(aes(x = log(Brain_Size_Species_Mean), y = pred_2[,2]), color = '90% Upper Band Prediction') +
  geom_line(aes(x = log(Brain_Size_Species_Mean), y = pred_2[,3], color = '90% Lower Band Prediction'))
...
``` {r Question 2.4}
# For normal values
predict(rline_1, newdata = data.frame(Brain_Size_Species_Mean = 800), interval = "prediction",
    level = 0.90)  # for a single value

# For log values
predict(rline_2, newdata = data.frame(Brain_Size_Species_Mean = 800), interval = "prediction",
    level = 0.90)  # for a single value
```

No, I do not trust these predictions. For one, the values of longevity predicted from 800 gr brain weight are all in the 1000s--and a 1000 month longevity is not likely If we look at the graphs our data points are all clustered around the 0-200 gr range. We should not be extrapolating the relationship for values outside of this range.
...
<br> 1) Plotting the lines based on a y=mx+b formula was difficult to figure out how to do!
<br> 2) The first function was logically hard for me to wrap my head around! I looked through the modules and had to google to make sure I was using the correct equations
<br> 3) I had some trouble fitting the predicition lines onto the plot--ggplot remains a difficult beast for me!

*3 ways your peers commentary helped you:*
<br> 1) I had missed the less and greater options for my two sided test! My peer correctly pointed that out
<br> 2) My peer correctly pointed out that I had left out prediciton lines on my plots!
<br> 3) My partner also tested me code and found a problem in my equation which was very helpful!!

**When I was Peer Commentating:** 

*2 things you liked in your peers code:*
<br> 1) My peer's stat's knowlegde I think was very strong. Stastically, they correctly completed everything asked of us
<br> 2) My peer used the predict() function even for the line plotting--I was doing it more manually so that was a better way!

*1 thing that could use improvement:*
<br> 1) My peer at times was using precalculated values instead of calculating them in the script--so I encouraged them to do that!
